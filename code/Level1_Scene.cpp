...
Scene* Level1_Scene::createScene()
{
	auto scene = Scene::create();

	auto layer = Level1_Scene::create();
	scene->addChild(layer);

	return scene;
}

void Level1_Scene::addLayer()
{
...
bool Level1_Scene::init()
{
	...
	addlevelscene();		//加载背景
	addLayer();		//加载图层
	GET_MUSIC_INSTANCE->playBackgroundMusic("music/bang.mp3", true); //加载背景音乐
	scheduleUpdate();

	return true;
}

void Level1_Scene::addlevelscene()
{
	Sprite *level1_bg1 = CCSprite::createWithSpriteFrameName("level1.png");
	level1_bg1->setTag(BG1);
	level1_bg1->setAnchorPoint(Vec2(0, 0));
	level1_bg1->setPosition(Vec2(0, 0));
	this->addChild(level1_bg1);

	Sprite * level1_bg2 = CCSprite::createWithSpriteFrameName("level1.png");
	...
	schedule(schedule_selector(Level1_Scene::moveLevelScene), 0.01f);
}

void Level1_Scene::moveLevelScene(float t)
{
...
//小敌机与玩家子弹碰撞
void Level1_Scene::Collision_smallEnemy_Mybullet()
{
...
//大敌机与玩家子弹碰撞
void Level1_Scene::Collision_bigEnemy_Mybullet()
{
...
//boss与玩家子弹碰撞
void Level1_Scene::Collision_bossEnemy_Mybullet()
{
...
void  Level1_Scene::Collision_smallEnemy_Myplane()
{
	if (Myplane::lifes >= 0)
	{
		Vector<Enemy*>* tempEnemyVec = new Vector<Enemy*>();
...
void Level1_Scene::Collision_bigEnemy_Myplane()
{
	if (Myplane::lifes >= 0)
	{
		Vector<Enemy*>* tempEnemyVec = new Vector<Enemy*>();
...
//boss 与玩家飞机碰撞
void Level1_Scene::Collision_bossEnemy_Myplane()
{
	if (Myplane::lifes >= 0)
	{
		Vector<Enemy*>* tempEnemyVec = new Vector<Enemy*>();
...
//敌机子弹与玩家飞机碰撞
void Level1_Scene::Collision_enemyBullet_Myplane()
{
	if (Myplane::lifes >= 0)
	{
		Vector<Sprite*>* tempBulletVec = new Vector<Sprite*>();
...
void Level1_Scene::Collision_Myplane_Food()
{
...
void Level1_Scene::update(float dt)
{
