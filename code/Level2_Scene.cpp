...
Scene* Level2_Scene::createScene()
{
	auto scene = Scene::create();

	auto layer = Level2_Scene::create();
	scene->addChild(layer);

	return scene;
}

void Level2_Scene::addLayer()
{
	_myplanelayer = MyplaneLayer::create();
	addChild(_myplanelayer, 2);
...
bool Level2_Scene::init()
{
	...
	addLayer();		//加载图层
	GET_MUSIC_INSTANCE->playBackgroundMusic("music/DJ.mp3", true); //加载背景音乐
...
void Level2_Scene::addlevelscene()
{
	Sprite *level1_bg1 = CCSprite::createWithSpriteFrameName("level2.png");
	level1_bg1->setTag(BG1);
	level1_bg1->setAnchorPoint(Vec2(0, 0));
	level1_bg1->setPosition(Vec2(0, 0));
	this->addChild(level1_bg1);

	Sprite * level1_bg2 = CCSprite::createWithSpriteFrameName("level2.png");
	...
	schedule(schedule_selector(Level2_Scene::moveLevelScene), 0.01f);
}

void Level2_Scene::moveLevelScene(float t)
{
...
//小敌机与玩家子弹碰撞
void Level2_Scene::Collision_smallEnemy_Mybullet()
{
...
//大敌机与玩家子弹碰撞
void Level2_Scene::Collision_bigEnemy_Mybullet()
{
...
//boss与玩家子弹碰撞
void Level2_Scene::Collision_bossEnemy_Mybullet()
{
...
void  Level2_Scene::Collision_smallEnemy_Myplane()
{
	if (Myplane::lifes >= 0)
	{
		Vector<Enemy*>* tempEnemyVec = new Vector<Enemy*>();
...
void Level2_Scene::Collision_bigEnemy_Myplane()
{
	if (Myplane::lifes >= 0)
	{
		Vector<Enemy*>* tempEnemyVec = new Vector<Enemy*>();
...
//boss 与玩家飞机碰撞
void Level2_Scene::Collision_bossEnemy_Myplane()
{
	if (Myplane::lifes >= 0)
	{
		Vector<Enemy*>* tempEnemyVec = new Vector<Enemy*>();
...
//敌机子弹与玩家飞机碰撞
void Level2_Scene::Collision_enemyBullet_Myplane()
{
	if (Myplane::lifes >= 0)
	{
		Vector<Sprite*>* tempBulletVec = new Vector<Sprite*>();
...
void Level2_Scene::Collision_Myplane_Food()
{
...
void Level2_Scene::update(float dt)
{
